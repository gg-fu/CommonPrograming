...
static char *select_color(const int level)
{
	char *color = NULL;
	switch(level)
	{
		case LOG_EMERG_LEVEL:
			color = REDBACK;
			break;
		case LOG_CRIT_LEVEL:
			color = L_RED;
...
void LITE_syslog_routine(char *m, const char *f, const int l, const int level, const char *fmt, va_list *params)
{
    ...
    color = select_color(level);
    LITE_printf(LOG_PREFIX_FMT,color, lvl_names[level], f, l);
