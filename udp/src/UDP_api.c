...
int UDP_write(intptr_t p_socket,
...
int UDP_read(intptr_t p_socket,
...
int UDP_readTimeout(intptr_t p_socket,
                        unsigned char *p_data,
                        unsigned int datalen,
                        unsigned int timeout)
{
    ...
    return UDP_read(p_socket, p_data, datalen);
}
...
intptr_t UDP_create_without_connect(_IN_ const char *host, _IN_ unsigned short port)
{
    ...
    if (NULL == host) {
        ...
        if (inet_aton(host, &in)) {
            ...
            if (!hp) {
                fprintf(stderr,"can't resolute the host address \n");
                close(sockfd);
                return -1;
            }
    ...
    printf("success to establish udp, fd=%d", sockfd);
...
int UDP_connect(_IN_ intptr_t sockfd,
...
int UDP_recv(_IN_ intptr_t sockfd,
...
int UDP_recvfrom(_IN_ intptr_t sockfd,         //////   UDP socket句柄
                     _OU_ NetworkAddr *p_remote,
                     _OU_ unsigned char *p_data,   /////   指向数据接收缓冲区的指针
                     _IN_ unsigned int datalen,   /////   接收缓冲区的字节大小
                     _IN_ unsigned int timeout_ms  )    /////   阻塞的超时时间, 单位ms
{
    int ret;
    struct sockaddr_in addr;
...
int UDP_send(  _IN_ intptr_t sockfd,        
...
int UDP_sendto(  _IN_ intptr_t sockfd,    /////   UDP socket句柄
                    _IN_ const NetworkAddr *p_remote,
                    _IN_ const unsigned char *p_data,   /////   指数据发送缓冲区的指针
                   _IN_ unsigned int datalen,   /////   待发送数据的字节长度
                   _IN_ unsigned int timeout_ms)    /////   阻塞的超时时间, 单位ms 
{
    int ret;
    uint32_t ip;
